...
export default function App({ preview = false }: { preview?: boolean }) {
  ...
  async function startNew(mode: "example" | "blank") {
    await perform(async () => {
      const m = await loadManifest(chosen);
      setManifest(m.manifest);
      setApproved(m.approved);
      setInputs(initialInputs(m.manifest, mode));
      setName(
        mode === "example"
          ? m.manifest.name + " example"
          : "Untitled calculation",
      );
      setCalculation(null);
      setRuns([]);
      setSelectedRun(null);
      setGroup(m.manifest.groups[0].id);
      setDirty(true);
  ...
  async function openCalculation(c: Calculation) {
    await perform(async () => {
      const [detail, m] = await Promise.all([
        request("/api/calculations/" + c.id),
        loadManifest(c.calculator_id, c.calculator_version),
      ]);
      setCalculation(detail.calculation);
      setName(detail.calculation.name);
      setInputs(detail.calculation.inputs);
      setRuns(detail.runs);
      setSelectedRun(null);
      setManifest(m.manifest);
      setApproved(m.approved);
  ...
  async function selectRun(run: Run) {
    await perform(async () => {
      const data = await request("/api/runs/" + run.id);
              ...
              <div className="billing-grid">
                {(["month", "year"] as const).map((interval) => (
                  <div className="card plan" key={interval}>
                    ...
                    <p className="subtitle">
                      One account. Full calculator access.
                    </p>
                    <button
                      className="btn primary"
                      style={{ marginTop: 22 }}
...
function Results({
  ...
  const [tab, setTab] = useState<"summary" | "detail" | "inputs">("summary");
